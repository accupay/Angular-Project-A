...
customerDetails: any;
  ...
  initBeneficiary() {
    ...
    this.dmtService
      .getCustomerApiPayOut(authCheckParam)
      .pipe(
        map(
        ...
        mergeMap((user) => {
          return this.dmtService.getBeneficiaryApiPayout({
  ...
  beneficiaryRegitrationSubmit() {
    ...
    this.dmtService.addPayeePayout(beneFormData).subscribe(
      (res: ResponseInterface) => {
        if (res.response_code === "200") {
  ...
  beneValidate(bene: any, type = "form") {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.dmtService.payeeValidatePayout(beneValidateParam).subscribe(
          (res: ResponseInterface) => {
  ...
  deleteBeneficiary(beneData: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        modalRef.componentInstance.beneDetail = beneData;
        modalRef.componentInstance.customerDetail = this.customerDetails;
        modalRef.componentInstance.agentNumber =this.retailerMobileNo;
        modalRef.componentInstance.deleteType ="PAYOUT";
        modalRef.componentInstance.deleteVerifyFlag =true;
  ...
  getAllBeneficiaryData(userId: any) {
    ...
    this.dmtService.getBeneficiaryApiPayout(authCheckParam).subscribe(
      (res: ResponseInterface) => {
        if (res.response_code === "200") {
          this.customerBeneficiaries = res.data;
          this.customerBeneficiariesSearch = res.data;
        ...
        (res: ResponseInterface) => {
          if (res.response_code === "200") {
            ...
            this.beneficiaryRegitrationForm.controls[
              ...
              Validators.pattern("^[A-Za-z0-9]{6}[0-9]*$"),
            ]);
            this.beneficiaryRegitrationForm.controls[
  ...
  searchTransaction() {
    ...
    this.dmtService.payoutTransactionReport(reportParam).subscribe(
      (res: ResponseInterface) => {
        if (res.response_code === "200") {
  ...
  changeClass(){
    ...
    if(disWidth <= 640) {
        ...
        return "container-fluid example";      
    } else {
