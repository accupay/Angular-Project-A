...
export class CustomerRegistrationComponent implements OnInit {
  ...
  customerRegitrationSubmit() {
    ...
    this.dmtService.addCustomerPayOut(customerAddData).subscribe(
  ...
  addCustomerOtpVerifyModal(customerResponse: any) {
    ...
    modalRef.componentInstance.customerIsInternal =
      customerResponse?.is_internal;
  ...
  pinCodeSubmit(value: any) {
    if (value.length === 6) {
      ...
      this.cacheService.getPinCodeApiPayout(authCheckParam).subscribe(
        (res: ResponseInterface) => {
          if (res.response_code === "200") {
