...
@Component({
  selector: "app-retailer-PG-report",
  templateUrl: "./retailer-PG-report.component.html",
  styleUrls: ["./retailer-PG-report.component.scss"],
  encapsulation: ViewEncapsulation.None,
})
export class retailerPGReportComponent
  implements OnInit, AfterViewInit
{
  ...
  searchTransaction() {
    ...
    this.loading = true;
    this.dmtService.getPgReport(reportParam).subscribe(
    ...
    if(filterData.length>0){
      filterData = filterData.filter(res =>
         (res.pg_link && res.pg_link.toLowerCase().indexOf(this.searchValues.toLowerCase()) > -1) ||
          ...
          (res.utR_number && res.utR_number.toLowerCase().indexOf(this.searchValues.toLowerCase()) > -1) ||
          (res.updated_date && res.updated_date.toLowerCase().indexOf(this.searchValues.toLowerCase()) > -1) ||
          (res.service_charge && res.service_charge.toLowerCase().indexOf(this.searchValues.toLowerCase()) > -1) ||
          ...
          (res.pg_status && res.pg_status.toLowerCase().indexOf(this.searchValues.toLowerCase()) > -1) ||
          (res.merchant_name && res.merchant_name.toLowerCase().indexOf(this.searchValues.toLowerCase()) > -1) ||
          (res.mobile && res.mobile.toLowerCase().indexOf(this.searchValues.toLowerCase()) > -1) ||
          (res.payment_id && res.payment_id.toLowerCase().indexOf(this.searchValues.toLowerCase()) > -1) ||
  ...
  exportToExcel() {
    const heyReport = this.dataSource.filteredData.map((data: any) => {
      return {
        ...
        "Retailer Mobile Number": data.mobile,
        "Retailer Name": data.merchant_name,
        "Amount": data.amount,
        // "Created Date": data.created_date,
        "Sevice Charge": data.service_charge,
        // "updated_date": data.updated_date,
        "Order Number": data.utR_number,
