...
@Component({
  selector: "app-payout-transaction-report",
  templateUrl: "./payout-transaction-report.component.html",
  styleUrls: ["./payout-transaction-report.component.scss"],
  encapsulation: ViewEncapsulation.None,
})
export class PayoutTransactionReportComponent implements OnInit, AfterViewInit {
  loading = false;
  loadingText = "Search";
    ...
    if(filterData.length>0){
      filterData = filterData.filter(res =>
        (res.transactionid.toLowerCase().indexOf(this.searchValues.toLowerCase()) > -1) ||
          ...
          (res.responsedescription.toLowerCase().indexOf(this.searchValues.toLowerCase()) > -1) ||
  ...
  exportToExcel() {
    const heyReport = this.dataSource.filteredData.map((data: any) => {
      return {
        ...
        "Response Code": data.responsedescription,
        "Amount": data.amount,
