...
@Component({
  selector: "app-refund-payment-transaction",
  templateUrl: "./refund-payment-transaction.component.html",
  styleUrls: ["./refund-payment-transaction.component.scss"],
  encapsulation: ViewEncapsulation.None,
})
export class RefundPaymentTransactionReportComponent
  implements OnInit, AfterViewInit
{
  ...
  searchTransaction() {
    ...
    this.loading = true;
    this.dmtService.refundPaymenttransactionReport(reportParam).subscribe(
  ...
  refund(transactionData: any) {
    ...
    const refundParam = {
      mobile_no: transactionData?.customermobileno,
      agent_ref_id: this.retailerRefId,
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.dmtService.resendOtpCRegistration(refundParam).subscribe(
          (res: ResponseInterface) => {
            this.loading = false;
            if (res.response_code === "200") {
              ...
              modalRef.componentInstance.refundType = "money-transfer";
  ...
  exportToExcel() {
    ...
    if (heyReport.length) {
      const fileName = "dmt_refund.xlsx";
